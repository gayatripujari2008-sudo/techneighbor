import streamlit as st   # ✅ Error fix: import sabse upar
...
# --- Sidebar Menu ---
menu = st.sidebar.radio("📌 Menu", ["Search Services", "Register as Provider"])

# --- Registration Form (only when selected) ---
if menu == "Register as Provider":
...
# --- Search Services (default view) ---
if menu == "Search Services":
    ...
    for s in st.session_state["services"]:
        if (not search_query or search_query.lower() in s["service"].lower()):
            if (not user_dgpin or user_dgpin == s["dgpin"]):
